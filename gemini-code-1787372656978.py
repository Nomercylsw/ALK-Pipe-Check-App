...
# --- 1. EPSG 좌표계 변환 설정 (CAD좌표 -> GPS 위경도) ---
# 5174(Bessel 중부원점)를 WGS84(GPS 기본 좌표계인 4326)로 변환
try:
...
if is_agreed:
    ...
    if contractor_file and st.button("간섭 및 이격거리 분석 실행"):
        if pipe_type == "선택 안함" or contractor_od <= 0 or contractor_depth <= 0:
            ...
            with st.spinner("CAD 도면 좌표 스캔 및 주소 변환 중..."):
                final_pipe_name = custom_pipe_type if pipe_type == "7. 기타" else pipe_type
                
                # --- 2. 주변 시설물 실제 추출 로직 (예시 구조) ---
                # 실제 ezdxf를 사용해 '2. 시설물' 레이어의 텍스트(TB33 등)의 X, Y 좌표를 추출합니다.
                # doc = ezdxf.read(contractor_file)
                # msp = doc.modelspace()
                # facilities = [{"name": text.dxf.text, "x": text.dxf.insert.x, "y": text.dxf.insert.y} for text in msp.query('TEXT MTEXT[layer=="2. 시설물"]')]
                
                # 테스트용 추출 결과라 가정 (좌표 단위: m)
                facilities = [
                    {"name": "TB33", "x": 204050, "y": 530050},
                    {"name": "GM17", "x": 204100, "y": 530100}
                ]
                
                # --- 3. 교차점 데이터 ---
                # 테스트 CAD 좌표 (안산 반월공단 부근의 중부원점 좌표 예시)
                clash_points = [
                    {"id": "P-1", "x": 204000, "y": 530000, "d_center": 250.0, "company_depth": 1500.0, "r_comp": 108.15}
                ]
                ...
                for cp in clash_points:
                    ...
                    # CAD 좌표(X, Y) -> 위경도(Lon, Lat) 변환
                    lon, lat = transformer.transform(cp["x"], cp["y"])
